...
def read_hidraw(fd):
    ...
    while len(hid_data) < HID_PACKET_SIZE:
        try:
            chunk = os.read(fd, HID_PACKET_SIZE - len(hid_data))
            if chunk == b'':  # This means EOF — the slave side is closed
                print("HIDRAW closed, exiting...")
                sys.exit(1)
    ...
    while len(hid_data) < (length+FRAME_HEADER_SIZE):
        try:
            chunk = os.read(fd, HID_PACKET_SIZE)
            if chunk == b'':  # This means EOF — the slave side is closed
                print("HIDRAW closed, exiting...")
                sys.exit(1)
        except BlockingIOError:
...
def main():
    ...
    # Open HID device
    hidraw_fd = os.open(hid_path, os.O_RDWR | os.O_NONBLOCK)
    ...
    while True:
        try:
            r_ready, _, _ = select.select([hidraw_fd, udp_sock], [], [])

            for fd in r_ready:
                if fd == hidraw_fd:
                    frame = read_hidraw(fd)
                    if frame:
                ...
                if fd == udp_sock:
                    ...
                    if frame:
                        ...
                        while len(frame) > 0:
                            ...
                            # Prepend dummy Report ID to data that is all zeroes
                            if chunk == b'\x00' * 64:
                                chunk = b'\x00' + chunk
